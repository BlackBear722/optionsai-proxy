...
async function runEngine() {
  ...
  var results = await Promise.all(watchlist.map(function(t){return scanTicker(t,settings);}));
  var signals = results.filter(function(r){return r.signal!=='NONE'&&(r.confidence==='HIGH'||r.confidence==='MEDIUM');});
  if (signals.length > 0) {
